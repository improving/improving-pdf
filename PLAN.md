...
- [x] Provide user-facing output guidance: path to generated PDF, optionally keep intermediate HTML.
...
- [x] Confirm Chromium auto-install triggers on first run.
- [x] Tag initial release for reproducible installs (`pip install ...@v1.0.0`).
...
- [x] Verify workflow prints install command for new version.
- [x] Verify workflow reminds to update pinned version in `skill.md` if applicable.
...
- [x] Test `/release` workflow: bump a patch version, verify tag and GitHub release are created.
